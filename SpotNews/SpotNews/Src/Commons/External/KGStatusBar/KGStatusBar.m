...
@synthesize topBar, overlayWindow, stringLabel,icon;
...
+ (void)showSuccessWithStatus:(NSString*)status
{
    ...
    [KGStatusBar performSelector:@selector(dismiss) withObject:self afterDelay:5];
}

+ (void)showWithStatus:(NSString*)status {
    [[KGStatusBar sharedView] showWithStatus:status barColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"NavBar64"]] textColor:[UIColor whiteColor]];
}
...
- (void)showWithStatus:(NSString *)status barColor:(UIColor*)barColor textColor:(UIColor*)textColor{
    ...
    //    }
    self.stringLabel.frame= CGRectMake(20, 0, SCREEN_WIDTH-10, 20);
...
- (UILabel *)stringLabel {
    if (stringLabel == nil) {
        ...
        stringLabel.font = [CommonUtils getFanTiFont:12];
        //		stringLabel.shadowColor = [UIColor blackColor];
        //		stringLabel.shadowOffset = CGSizeMake(0, -1);
        stringLabel.numberOfLines = 0;
        
        
        self.icon=[[UIImageView alloc] initWithFrame:CGRectMake(5, 5, 10, 10)];
        icon.image=[UIImage imageNamed:@"statusBarLogo"];
